...
result['center_diff'] = result.apply(lambda row: math.sqrt(
    pow((row.x1_x - row.x1_y), 2)+pow((row.y_x - row.y_y), 2)), axis=1)
result['center_angle'] = result.apply(lambda row: np.rad2deg(
    np.arctan2((row.y_x - row.y_y), (row.x1_x - row.x1_y))), axis=1)
result = result.drop(columns=['x2_x', 'y1_x', 'y2_x', 'width_x', 'height_x',
                              'x1_y', 'x2_y', 'y1_y', 'y2_y', 'width_y',
                              'height_y'])
result['same_row'] = result.apply(lambda row: '1' if abs(
    row.center_angle) > 177 and abs(row.center_angle) < 183 else '0', axis=1)
result['same_column'] = result.apply(lambda row: '1' if abs(
    row.center_angle) > 86 and abs(row.center_angle) < 94 else '0', axis=1)
...
only_id = result.drop(columns=['x1_x', 'inside_x', 'y_x', 'inside_y', 'y_y',
                               'center_diff', 'center_angle', 'same_row', 'same_column', 'ndist'])
# with pd.option_context('display.max_rows', None, 'display.max_columns', None):  # more options can be specified also
#     print(result)
result = result.drop(columns=['x1_x', '_id_x', 'y_x', '_id_y', 'y_y',
                              'center_diff', 'center_angle'])
print(result)

clustering = DBSCAN(eps=0.5, min_samples=2).fit(result)
print(clustering.labels_)
for x in len(set(clustering.labels_)):
    print("hello")
...
for n_clusters in range(numClustersStart, numClustersEnd, step):
    ...
    visualizer.show(outpath="./" + opath + "/" +
                    str(n_clusters) + ".png")  # TODO
    plt.cla()
    plt.clf()
    plt.close("all")
    print(
