...
result['center_diff'] = result.apply(lambda row: math.sqrt(
    pow((row.x_x - row.x_y), 2)+pow((row.y_x - row.y_y), 2)), axis=1)
result['center_angle'] = result.apply(lambda row: np.rad2deg(
    np.arctan2((row.y_x - row.y_y), (row.x_x - row.x_y))), axis=1)
result = result.drop(columns=['x1_x', 'x2_x', 'y1_x', 'y2_x', 'width_x', 'height_x', 'x_x', 'y_x',
                     'x1_y', 'x2_y', 'y1_y', 'y2_y','width_y', 'height_y', 'x_y', 'y_y'])
